  ...
  <v-container>
    ...
    <v-card-actions>
      ...
      <v-btn
        ...
        @click="saveDialog()"
        :disabled="
          manual_review.root_cause_id == null && manual_review.manual_result == 2
        "
      >
        Salvar
      </v-btn>
...
export default {
  ...
  methods: {
    ...
    fetchRootCauses(payload) {
      this.dialog = true;
      let uri = `${config.AWS_URL}/rootcauses_coverage`;
      if (payload) {
    ...
    saveDialog() {
      if (this.$refs.form.validate()) {
        ...
        this.dialog = true;
        let url = `${config.AWS_URL}/manual_review_validation`;
